...
struct Vertex {
    ...
    static std::array<VkVertexInputAttributeDescription, 2> getAttributeDescriptions() {
        ...
        attributeDescriptions[0].offset = offsetof(Vertex, pos); //개쩐다 이런게 있네
...
private:
    ...
    void drawFrame()
    {
        ...
        // 1. 스왑체인에서 이미지(Image)를 가져온다.
        // 2. 프레임버퍼에서 해당 이미지를 첨부파일로 사용하여 커맨드 버퍼를 실행한다.
        // 3. 프레젠테이션을 위해서 이미지를 스왑 체인으로 반환한다.
    ...
    void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkBufferUsageFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
    {
    ...
    void recreateSwapChain(){
        ...
        vkDeviceWaitIdle(device); // 사용 중인 리소스를 건들면 안됨. (Logical Device의 큐 작업이 완료될때까지 대기)
